<?php
  ...
  class bestSellersList{
    ...
    //constructors
    function bestSellersList($idCategory,$idLanguage,$idGroup,$link){
      ...
      $requestString="SELECT $TBL_catalogue_info_products.name,
                ...
                $TBL_catalogue_products.new_product,
                $TBL_catalogue_products.best_seller,
          ...
          WHERE $TBL_catalogue_products.best_seller='Y'
      ...
      while($row=mysql_fetch_object($request)){
        ...
        $this->productTable[$row->id]["is_new"]=$row->new_product=="Y";
        $this->productTable[$row->id]["is_bestseller"]=$row->best_seller=="Y";
