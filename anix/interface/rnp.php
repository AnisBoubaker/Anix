...
Class RNPEncodingKey {
    var $clMD5;

    function RNPEncodingKey() {
    ...
    function giveHashCode($pkey, $second, $email, $refid, $montant, $nom) {
        ...
        return $this->clMD5->hash($pkey.$refid.trim($montant).trim($email).$select);

    }

    function genchecksum($xmlinfo) {
        return $this->clMD5->hash($xmlinfo);
    }
}
?>
