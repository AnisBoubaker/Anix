...
function ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$gap,$size,&$y,$optionsAll=array()){
  ...
  if(isset($optionsAll['textCol'])){
    $mainColor = $optionsAll['textCol'];
  } else $mainColor = 0;
...
//  $y-=$gap-$decender;
  ...
  while ($ok==0){
    foreach($cols as $colName=>$colHeading){
      $this->ezSetY($y);
      //Anis
      if(isset($options[$colName]['color']) && is_array($options[$colName]['color']) && is_array($mainColor)) {
        $this->setColor($options[$colName]['color'][0],$options[$colName]['color'][1],$options[$colName]['color'][2]);
      }
      ...
      //Anis
      if(isset($options[$colName]['color']) && is_array($options[$colName]['color']) && is_array($mainColor)) {
          $this->setColor($mainColor[0],$mainColor[1],$mainColor[2]);
      }
      $dy = $y-$this->y;
      if ($dy>$mx){
        $mx=$dy;
...
//        $x1=$x1+$dm;
  ...
  //            $this->ezNewPage();
            ...
            if ($colNewPage){
              ...
              //Anis
              if(isset($options['cols'][$colName]['color']) && is_array($options['cols'][$colName]['color'])) {
                $this->setColor($options['cols'][$colName]['color'][0],$options['cols'][$colName]['color'][1],$options['cols'][$colName]['color'][2]);
              }
                 //$this->setColor($row[$options['cols'][$colName]['color']][0],$row[$options['cols'][$colName]['color']][1],$row[$options['cols'][$colName]['color']][2]);
              //Anis for bgcolor and elseif
            if(isset($options['cols'][$colName]['bgcolor']) && is_array($options['cols'][$colName]['bgcolor'])) {
                $this->setStrokeColor($options['cols'][$colName]['bgcolor'][0],$options['cols'][$colName]['bgcolor'][1],$options['cols'][$colName]['bgcolor'][2],1);
            }
