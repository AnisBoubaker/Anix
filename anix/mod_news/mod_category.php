...
switch($action){
	...
	case "insert":setTitleBar(_("Ajout d'une catégorie de nouvelles"));break;
	case "edit":setTitleBar(_("Modification d'une catégories de nouvelles"));break;
	case "update":setTitleBar(_("Modification d'une catégories de nouvelles"));break;
	default:setTitleBar(_("Modification d'une catégories de nouvelles"));
}
?>
...
<td colspan='2'>
  ...
  <tr>
    ...
    <td>
    <?php
    $result = request("SELECT * FROM `$TBL_news_categories` WHERE `id`='$idCat'",$link);
...
<?
if($action=="edit"){
	$result=request("SELECT id,id_parent,description,name,htmltitle,htmldescription,keywords from $TBL_news_info_categories,$TBL_news_categories where id='$idCat' and id=id_news_cat and id_language='".$row_languages->id."'",$link);
	...
	if(!mysql_num_rows($result)) die("Erreur de protection: Cette cat�orie de nouvelles n'existe pas.");
	$edit = mysql_fetch_object($result);
}
?>
