    ...
    <tr>
      <td colspan='2'>
        ...
        <tr>
          ...
          if($action=="search_news") echo "value='".$_POST["title"]."'";
          ?>></td>
        </tr>
        <tr>
          ...
          <td><SELECT name='category'>
        	  ...
        	  foreach($listCategories as $row){
        	  	...
        	  	if($action=="search_news" && $_POST["category"]==$row["id"]) echo " SELECTED";
        ...
        <tr>
          ...
          if($action=="search_news") echo "value='".$_POST["keywords"]."'";
          ?>></td>
        </tr>
        <tr>
          <td valign='top'><?php echo _("Filtres"); ?>:</td>
          <td><input type='checkbox' name='active' <?
          if($action=="search_news" && isset($_POST["active"])) echo "CHECKED";
          ?>> <?php echo _("Actives"); ?>
          &nbsp;&nbsp;
          <input type='checkbox' name='disactivated' <?
          if($action=="search_news" && isset($_POST["disactivated"])) echo "CHECKED";
          ?>> <?php echo _("Désactivées"); ?><br>
          <input type='checkbox' name='awaiting' <?
          if($action=="search_news" && isset($_POST["awaiting"])) echo "CHECKED";
          ?>> <?php echo _("En attente"); ?>
          &nbsp;&nbsp;
          <input type='checkbox' name='expired' <?
          if($action=="search_news" && isset($_POST["expired"])) echo "CHECKED";
          ?>> <?php echo _("Expirées"); ?><br>
          <input type='checkbox' name='archived' <?
          if($action=="search_news" && isset($_POST["archived"])) echo "CHECKED";
...
<?
if(($action=="search_news" || $action=="search_category") && $nb){
?>
  <table border="0" align="center" width="95%" bgcolor='#FFFFFF' CellPadding="0" CellSpacing="0">
  <tr height='20'>
    <td  background='../images/button_back.jpg' align='left' valign='middle'>
  ...
  if($nbResults){
  	while($result=mysql_fetch_object($request)){
  		if($action=="search_news"){
  			echo "<tr>";
  			echo "<td valign='middle' width='42' bgcolor='#e7eff2' align='right'><a href='./mod_news.php?action=edit&idNews=".$result->id."'><img src='../images/edit.gif' border='0' alt=\""._("Modifier")."\"></a></td>";
