...
$menu_ouvert = 2;$module_name="news";include("../html_header.php");
?>
<form id='main_form' action='./mod_news.php' method='POST' enctype='multipart/form-data' name='mainForm'>
<?
...
if($action=="edit" || $action=="update"){
	...
	$result=request("SELECT * from $TBL_news_news where id='$idNews'",$link);
	//echo "SELECT id,id_parent,description,name from $TBL_news_info_categories,$TBL_news_categories where id_news_cat='$idCat' and id_language='".$row_languages->id."'";
...
$cancelLink="./list_news.php?action=edit&idCat=$idCat";
switch($action){
...
<tr>
  <td valign="top">
  	...
  	if($action=="add" || $action=="insert"){
  		echo "<center><a href='../$CATALOG_folder_images/imgnews_large_no_image.jpg' target='_blank'><img class='item_image' src='../$CATALOG_folder_images/imgnews_small_no_image.jpg' alt=\""._("Agrandir")."\"></a></center><br>";
  	}
  	?>
  	<table>
  	<tr>
...
if($action=="edit"){
	$result=request("SELECT id,id_category,active,from_date,to_date,title,date,short_desc,details,keywords,htmltitle,htmldescription from $TBL_news_info_news,$TBL_news_news where id='$idNews' and id=id_news and id_language='".$row_languages->id."'",$link);
	//echo "SELECT id,id_parent,description,name from $TBL_news_info_categories,$TBL_news_categories where id_news_cat='$idCat' and id_language='".$row_languages->id."'";
	if(!mysql_num_rows($result)) die(_("Erreur de protection: Cette nouvelle n'existe pas."));
	$edit = mysql_fetch_object($result);
	$idCat=$edit->id_category;
