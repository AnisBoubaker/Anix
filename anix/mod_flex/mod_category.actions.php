...
if($action=="insert"){
	...
	$requestString="INSERT INTO `$TBL_lists_categories` (`ordering`, `id_parent`,`image_file_large`,`image_file_small`,`contain_items`,`id_menu`,`itemimg_icon_width`,`itemimg_icon_height`,`itemimg_small_width`,`itemimg_small_height`,`itemimg_large_width`,`itemimg_large_height`,`items_ordering`,`subcats_ordering`,`created_on`,`created_by`,`modified_on`,`modified_by`) values ('$ordering','$idCat','imgcatflex_large_no_image.jpg','imgcatflex_small_no_image.jpg','".(isset($_POST["contain_items"])?"Y":"N")."','".$_POST["id_menu"]."','".$_POST["itemimg_icon_width"]."','".$_POST["itemimg_icon_height"]."','".$_POST["itemimg_small_width"]."','".$_POST["itemimg_small_height"]."','".$_POST["itemimg_large_width"]."','".$_POST["itemimg_large_height"]."','".$_POST["items_ordering"]."','".$_POST["subcats_ordering"]."','".getDBDate()."','$anix_username','".getDBDate()."','$anix_username')";
	...
	if(!mysql_errno($link)) {
		...
		$errMessage.=_("Une erreur s'est produite lors de l'insertion de la catégorie d'éléments.")."<br />";
		$errors++;
	}
	...
	if(!$errors){
		...
		while (!$errors && $row_languages=mysql_fetch_object($languages)){
			$requestString="INSERT INTO `$TBL_lists_info_categories` (`id_lists_cat`,`id_language`,`name`,`description`,`keywords`,`htmltitle`,`htmldescription`) values (";
			$requestString.="'$idCat',";
	...
	if(!$errors && $_POST["image_action"]=="change"){
		...
		if($imageUploaded){
			$fileName = 'imgcatflex_'.$idCat.'_'.$_FILES['image_file']['name'];
			$fileName_large = 'imgcatflex_large_'.$idCat.'_'.$_FILES['image_file']['name'];
			$fileName_small = 'imgcatflex_small_'.$idCat.'_'.$_FILES['image_file']['name'];
		...
		if($imageUploaded){
			$requestString ="UPDATE `$TBL_lists_categories` SET ";
...
if($action=="update"){
	...
	//load the old values
	$old_values = mysql_fetch_object($result);

	$languages=request("SELECT id FROM `$TBL_gen_languages` WHERE used = 'Y'", $link);
	while (!$errors && $row_languages=mysql_fetch_object($languages)){
		$requestString="UPDATE `$TBL_lists_info_categories` set ";
		...
		$requestString.=" WHERE `id_lists_cat`='$idCat' and id_language='".$row_languages->id."'";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors && $_POST["image_action"]=="delete"){
		...
		$fileName_large="imgcatflex_large_no_image.jpg";
		$fileName_small="imgcatflex_small_no_image.jpg";
	}
	//Update the category
	if(!$errors){
		$requestString ="UPDATE `$TBL_lists_categories` SET ";
		...
		$requestString .="`itemimg_icon_width`='".$_POST["itemimg_icon_width"]."',";
		$requestString .="`itemimg_icon_height`='".$_POST["itemimg_icon_height"]."',";
		$requestString .="`itemimg_small_width`='".$_POST["itemimg_small_width"]."',";
		$requestString .="`itemimg_small_height`='".$_POST["itemimg_small_height"]."',";
		$requestString .="`itemimg_large_width`='".$_POST["itemimg_large_width"]."',";
		$requestString .="`itemimg_large_height`='".$_POST["itemimg_large_height"]."',";
		$requestString .="`items_ordering`='".$_POST["items_ordering"]."',";
		$requestString .="`subcats_ordering`='".$_POST["subcats_ordering"]."',";
	...
	if(!$errors){
		$extraSections=request("SELECT $TBL_lists_extracategorysection.id,$TBL_lists_info_extracategorysection.id_language from $TBL_lists_extracategorysection,$TBL_lists_info_extracategorysection where id_cat='$idCat' and id=id_extrasection",$link);
		while($extraSection=mysql_fetch_object($extraSections)){
			$requestString ="UPDATE `$TBL_lists_info_extracategorysection` SET";
...
if($action=="addExtraField"){
	...
	if(!$errors){
		...
		$requestString="INSERT INTO `$TBL_lists_extrafields` (`datatype`,`id_cat`,`params`,`ordering`) values ('".$_POST["type"]."','$idCat','$params','$maxOrderValue')";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		...
		while($language=mysql_fetch_object($languages)){
			...
			$requestString="INSERT INTO `$TBL_lists_info_extrafields` (`id_extrafield`,`id_language`,`name`,`description`, `selection_values`) values ('$idExtraField','".$language->id."','$name','$description','$selectionValue')";
			request($requestString,$link);
			if(mysql_errno($link)){
	...
	if(!$errors){
		request("UPDATE $TBL_lists_categories SET modified_on='".getDBDate()."',modified_by='$anix_username' WHERE id='$idCat'",$link);
	}
	if(!$errors) {
...
if($action=="updateExtraField"){
	...
	if($_POST["datatype"]=="text"){
		$params=$_POST["param1"];
		if($params=="") $params=50;
		$requestString ="UPDATE `$TBL_lists_extrafields` SET ";
	...
	if(!$errors){
		$languages=request("SELECT * FROM `$TBL_gen_languages` WHERE used ='Y'",$link);
		while($language=mysql_fetch_object($languages)){
			$requestString ="UPDATE `$TBL_lists_info_extrafields` SET ";
...
if($action=="deleteExtraField"){
	$idExtraField=$_POST["idExtraField"];
	$request = request("SELECT id,deletable,ordering from `$TBL_lists_extrafields` where id='$idExtraField'",$link);
	...
	if(!$errors){
		$requestString = "DELETE from `$TBL_lists_extrafields_values` where id_extrafield='$idExtraField'";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		$requestString = "DELETE from `$TBL_lists_info_extrafields` where id_extrafield='$idExtraField'";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		$requestString = "DELETE from `$TBL_lists_extrafields` where id='$idExtraField'";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		request("UPDATE `$TBL_lists_extrafields` set ordering=ordering-1 where id_cat='$idCat' and ordering > ".$extrafield->ordering,$link);
		if(mysql_errno($link)){
			$errors++;
...
if($action=="addExtraSection"){
	if(!$errors){
		...
		$requestString="INSERT INTO `$TBL_lists_extracategorysection` (`id_cat`,`ordering`) values ('$idCat','$maxOrderValue')";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		...
		while($language=mysql_fetch_object($languages)){
			...
			$requestString="INSERT INTO `$TBL_lists_info_extracategorysection` (`id_extrasection`,`id_language`,`name`, `value`) values ('$idExtraSection','".$language->id."','$name','$value')";
			request($requestString,$link);
			if(mysql_errno($link)){
...
if($action=="deleteExtraSection"){
	...
	$request = request("SELECT id,deletable,ordering from `$TBL_lists_extracategorysection` where id='$idExtraSection'",$link);
	if(mysql_num_rows($request)){
	...
	if(!$errors){
		$requestString = "DELETE from `$TBL_lists_info_extracategorysection` where id_extrasection=$idExtraSection";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	if(!$errors){
		$requestString = "DELETE from `$TBL_lists_extracategorysection` where id=$idExtraSection";
		request($requestString,$link);
		if(mysql_errno($link)) {
	...
	//update the orderings
	if(!$errors){
		request("UPDATE `$TBL_lists_extracategorysection` set ordering=ordering-1 where id_cat='$idCat' and ordering > ".$extrasection->ordering,$link);
		if(mysql_errno($link)){
			$errors++;
...
if($action=="updateExtraSection"){
	...
	while($language=mysql_fetch_object($languages)){
		$name = htmlentities($_POST["name_".$language->id],ENT_QUOTES,"UTF-8");
		$requestString="UPDATE `$TBL_lists_info_extracategorysection` SET `name`='$name' WHERE `id_extrasection`='$idExtraSection' and `id_language`='".$language->id."'";
		request($requestString,$link);
		if(mysql_errno($link)){
			$errMessage.=_("Une erreur s'est produite lors de la mise à jour des informations de la section additionnelle.")."<br />";
			$errors++;
...
if($action=="addAttachment"){
	...
	$requestString ="INSERT INTO `$TBL_lists_attachments` (`id_category`,`id_language`,`title`,`description`,`ordering`) values (";
	$requestString.="'$idCat',";
...
if($action=="delAttachment"){
	...
	//update the orderings
	if(!$errors){
		request("UPDATE `$TBL_lists_attachments` set ordering=ordering-1 where id_category='$idCat' and ordering > ".$oldFile->ordering,$link);
		if(mysql_errno($link)){
			$errors++;
...
if($action=="moveAttachmentUp"){
	...
	if(!$errors){
		$request=request("SELECT id,ordering from `$TBL_lists_attachments` where id_category='$idCat' and ordering='".($attachment->ordering-1)."'",$link);
		if(mysql_num_rows($request)){
...
if($action=="moveEFup"){
	...
	if(!$errors){
		$request=request("SELECT id,ordering from $TBL_lists_extrafields where id='$idExtrafield'",$link);
		if(mysql_num_rows($request)){
			$extrafield=mysql_fetch_object($request);
		} else {
			$errors++;
	...
	if(!$errors){
		$request=request("SELECT id,ordering from $TBL_lists_extrafields where id_cat='$idCat' and ordering='".($extrafield->ordering-1)."'",$link);
		if(mysql_num_rows($request)){
	...
	if(!$errors){
		request("UPDATE $TBL_lists_extrafields set ordering='".($extrafield->ordering-1)."' where id='$idExtrafield'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=("Une erreur s'est élémentlors de la mise à jour de l'ordre du champs additionnel.")."<br />";
		}
	}
	if(!$errors){
		request("UPDATE $TBL_lists_extrafields set ordering='".$extrafield->ordering."' where id='".$upField->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est élémentlors de la mise à jour de l'ordre du champs additionnel.")."<br />";
		}
	}
	if(!$errors){
		request("UPDATE $TBL_lists_categories set modified_on='".getDBDate()."',modified_by='$anix_username' where id='$idCat'",$link);
...
if($action=="moveEFdown"){
	...
	if(!$errors){
		request("UPDATE $TBL_lists_extrafields set ordering='".$extrafield->ordering."' where id='".$downField->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=("Une erreur s'est élémentlors de la mise à jour de l'ordre du champs additionnel.")."<br />";
		}
...
if($action=="moveESup"){
	...
	if(!$errors){
		$request=request("SELECT id,ordering from `$TBL_lists_extracategorysection` where id='$idExtrasection'",$link);
		if(mysql_num_rows($request)){
			$extrasection=mysql_fetch_object($request);
		} else {
			$errors++;
	...
	if(!$errors){
		$request=request("SELECT id,ordering from `$TBL_lists_extracategorysection` where id_cat='$idCat' and ordering='".($extrasection->ordering-1)."'",$link);
		if(mysql_num_rows($request)){
	...
	if(!$errors){
		request("UPDATE `$TBL_lists_extracategorysection` set ordering='".($extrasection->ordering-1)."' where id='$idExtrasection'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est élémentlors de la mise à jour de l'ordre de la section additionnelle.")."<br />";
		}
	}
	if(!$errors){
		request("UPDATE `$TBL_lists_extracategorysection` set ordering='".$extrasection->ordering."' where id='".$upSection->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est élémentlors de la mise à jour de l'ordre de la section additionnelle.")."<br />";
		}
...
if($action=="moveESdown"){
	...
	if(!$errors){
		request("UPDATE `$TBL_lists_extracategorysection` set ordering='".$extrasection->ordering."' where id='".$downSection->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est élémentlors de la mise à jour de l'ordre de la section additionnelle.")."<br />";
		}
