...
<td colspan='2'>
  ...
  <tr>
    ...
    <td>
    <?php
    $result = request("SELECT * FROM `$TBL_gallery_categories` WHERE `id`='$idCat'",$link);
...
<?
if($action=="edit"){
	$result=request("SELECT id,id_parent,description,name,htmltitle,htmldescription,keywords from $TBL_gallery_info_categories,$TBL_gallery_categories where id='$idCat' and id=id_gallery_cat and id_language='".$row_languages->id."'",$link);
...
?>
  ...
  <font class='fieldTitle'><?php echo _("Description"); ?>:</font><br>
  	...
  	if($action=="add"){
  	$oFCKeditor->Value = $PHOTO_editor_default_value;
  	}
  	if($action=="edit"){
