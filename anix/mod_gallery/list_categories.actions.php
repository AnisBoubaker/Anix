...
if($action=="moveup"){
	...
	if(!$ANIX_messages->nbErrors){
		$request=request("SELECT * from $TBL_gallery_categories where id_parent='".$category->id_parent."' and ordering='".($category->ordering-1)."'",$link);
		if(mysql_num_rows($request)){
	...
	if(!$ANIX_messages->nbErrors){
		request("UPDATE $TBL_gallery_categories set ordering='".($category->ordering-1)."',modified_on='".getDBDate()."',modified_by='$anix_username' where id='".$category->id."'",$link);
		if(mysql_errno()){
			$ANIX_messages->addError(_("Une erreur s'est produite lors de la mise à jour de l'ordre de la catégorie."));
		}
	}
	if(!$ANIX_messages->nbErrors){
		request("UPDATE $TBL_gallery_categories set ordering='".$category->ordering."' where id='".$upCategory->id."'",$link);
		if(mysql_errno()){
			$ANIX_messages->addError(_("Une erreur s'est produite lors de la mise à jour de l'ordre de la catégorie."));
		}
	}
...
if($action=="movedown"){
	...
	if(!$ANIX_messages->nbErrors){
		request("UPDATE $TBL_gallery_categories set ordering='".$category->ordering."' where id='".$downCategory->id."'",$link);
		if(mysql_errno()){
			$ANIX_messages->addError(_("Une erreur s'est produite lors de la mise à jour de l'ordre de la catégorie."));
		}
	}
...
if($action=="deletecat"){
	...
	if(!$ANIX_messages->nbErrors){
		$request=request("SELECT deletable,ordering,id_parent from $TBL_gallery_categories where id=$idCat",$link);
		if(mysql_num_rows($request)){
	...
	if(!$ANIX_messages->nbErrors && isset($_POST["method"]) && $_POST["method"]=="delete"){
		...
		if(!$ANIX_messages->nbErrors){
			...
			request("UPDATE $TBL_gallery_categories set ordering=ordering+$maxCatOrder where id_parent='$idCat'",$link);
			// Move the subcategories to the target
			request("UPDATE $TBL_gallery_categories set id_parent='$moveto' where id_parent='$idCat'",$link);
			if(mysql_errno($link)){
				$ANIX_messages->addError(_("Une erreur s'est produite lors du déplacement des sous-catégories"));
			}
		}
		if(!$ANIX_messages->nbErrors){
			//get the target category max photo ordering
			$maxPhotosOrder= getMaxPhotoOrder($moveto,$link)-1;
			//update the ordering
			request("UPDATE $TBL_gallery_photo set ordering=ordering+$maxPhotosOrder where id_category=$idCat",$link);
			//Move the photo to the photo category
			request("UPDATE $TBL_gallery_photo set id_category=$moveto where id_category=$idCat",$link);
			if(mysql_errno($link)){
		...
		if(!$ANIX_messages->nbErrors){
			...
			request("UPDATE $TBL_gallery_categories set ordering=ordering-1 where id_parent=$catParent and ordering > $catOrdering",$link);
			if(mysql_errno($link)){
		...
		if(!$ANIX_messages->nbErrors){
			//Delete the category
			request("DELETE from $TBL_gallery_info_categories where id_gallery_cat='$idCat'",$link);
			if(mysql_errno($link)){
		...
		if(!$ANIX_messages->nbErrors){
			//Delete the category
			request("DELETE from $TBL_gallery_categories where id=$idCat",$link);
			if(mysql_errno($link)){
