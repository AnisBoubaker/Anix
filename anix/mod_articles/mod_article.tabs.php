...
?>
    ...
    ?>> <?php echo _("Cet article est toujours affiché.");?><br>
    ...
    ?>> <?php echo _("Afficher cet article du"); ?> <input type='text' name='from_date' id='from_date' size='10'<?
    ...
    ?>> <?php echo _("Cet article est archivé (affichée avec les archives).");?><br>
    ...
    ?>> <?php echo _("Cet article est désactivé (non affiché)."); ?><br /><br />
    <input type='checkbox' name='home_page' <?php
    if($action=="edit" && $edit->home_page=="Y") echo " checked='checked'";
    if(($action=="insert" || $action=="update") && $_POST["home_page"]=="Y") echo " checked='checked'";
    ?>>&nbsp;<font class='fieldTitle'><?php echo _("Afficher sur la page d'accueil"); ?></font><br>
<?php
TABS_closeTab();
/**
 * TAB2: LINKS
...
?>
      ...
      <?
      //item attachments
      $attachments=request("SELECT $TBL_articles_attachments.id id, $TBL_articles_attachments.title attachment,$TBL_gen_languages.name language,$TBL_articles_attachments.ordering FROM `$TBL_articles_attachments`,`$TBL_gen_languages` WHERE $TBL_articles_attachments.id_article='$idArticle' AND $TBL_articles_attachments.id_language=$TBL_gen_languages.id order by $TBL_articles_attachments.ordering",$link);
      if(mysql_num_rows($attachments)>0){
      	...
      	while($attachment = mysql_fetch_object($attachments)){
      		...
      		if($attachment->ordering>1){
      			echo "<a href='./mod_article.php?idArticle=$idArticle&idAttachment=".$attachment->id."&action=moveAttachmentUp'><img src='../images/order_up.gif' border='0' alt=\""._("Monter")."\"></a>&nbsp;";
      		} else echo "<img src='../images/order_blank.gif' border='0'>&nbsp;";
      		if($attachment->ordering<$maxOrderValue){
      			echo "<a href='./mod_article.php?idArticle=$idArticle&idAttachment=".$attachment->id."&action=moveAttachmentDown'><img src='../images/order_down.gif' border='0' alt=\""._("Descendre")."\"></a>&nbsp;";
      		} else echo "<img src='../images/order_blank.gif' border='0'>&nbsp;";
      		echo "<a href='./mod_attachment.php?action=edit&idArticle=$idArticle&idAttachment=".$attachment->id."'><img src='../images/edit.gif' border='0' alt=\""._("Modifier")."\"></a>";
      		echo "&nbsp;<a href='./del_attachment.php?idAttachment=".$attachment->id."&idArticle=$idArticle'><img src='../images/del.gif' border='0' alt=\""._("Supprimer")."\"></a>";
      		echo "</td>";
      		echo "</tr>";
      	}
      } else {
     ...
     ?>
      <tr>
        <td NOWRAP align='right' colspan='2'><A href='./mod_attachment.php?action=add&idArticle=<?=$idArticle?>'><?php echo _("Ajouter"); ?></A></td>
      </tr>
      <?
      } //IF
      ?>
