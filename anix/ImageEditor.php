...
class ImageEditor {
  ...
  ##############################################################
  # RESIZE IMAGE GIVEN maxX AND maxY (keep original proportions)
  ##############################################################
  function resize($width, $height)
  {
    if(!$this->error)
    {
	  if( (($this->x) > $width) || (($this->y) > $height)){
	    //Compute undisortioned image size (By Anis)
	    //$k = ( $this->x >= $this->y ) ? $this->x / $width : $this->y / $height;
	    $k = max($this->x / $width, $this->y/$height);
	    $newWidth=floor($this->x/$k);
	    $newHeight=floor($this->y/$k);
	  }	else{
	  	$newWidth=$this->x;
	    $newHeight=$this->y;
	  }
      ...
      imagecopyresampled($tmpimage, $this->img, 0, 0, 0, 0,
                           $newWidth, $newHeight, $this->x, $this->y);
  ...
  ###########################################################
  # RESIZE IMAGE GIVEN X AND Y (doesn't keep the proportions)
  ###########################################################
  ...
  function outputFile($filename, $path)
  {
    if ($this->type == 'jpg' || $this->type == 'jpeg')
    {
      imagejpeg($this->img, ($path . $filename), 100);
    }
    else if ($this->type == 'png')
    {
