...
?>
<?
$result=request("select $TBL_catalogue_attachments.id,$TBL_catalogue_attachments.title,$TBL_gen_languages.name from $TBL_catalogue_attachments,$TBL_gen_languages where $TBL_catalogue_attachments.id='$idAttachment' and $TBL_gen_languages.id=$TBL_catalogue_attachments.id_language", $link);
$attachment=mysql_fetch_object($result);
if($idCategory) {
	$cancelLink="./mod_category.php?idCat=$idCategory&action=edit";
}elseif($idProduct) {
	$cancelLink="./mod_product.php?idProduct=$idProduct&action=edit";
}
$button=array();
