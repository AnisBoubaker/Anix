...
switch($action){
	case "add":setTitleBar(_("Ajout d'un champs additionnel de produits"));break;
	case "insert":setTitleBar(_("Ajout d'un champs additionnel de produits"));break;
	case "edit":setTitleBar(_("Modification d'un champs additionnel de produits"));break;
	case "update":setTitleBar(_("Modification d'un champs additionnel de produits"));break;
	default:setTitleBar(_("Modification d'un champs additionnel de produits"));break;
}
?>
...
<td colspan='2'>
    ...
    <?
    if($action=="edit"){
    	$languages=request("SELECT $TBL_gen_languages.image_file,$TBL_gen_languages.locales_folder,$TBL_gen_languages.id,$TBL_catalogue_info_extrafields.name extrafield FROM `$TBL_gen_languages`,`$TBL_catalogue_info_extrafields` WHERE $TBL_gen_languages.used ='Y' and $TBL_catalogue_info_extrafields.id_extrafield=$idExtraField and $TBL_catalogue_info_extrafields.id_language=$TBL_gen_languages.id ORDER BY $TBL_gen_languages.id!='$used_language_id',$TBL_gen_languages.default",$link);
    } elseif($action=="add") {
  ...
  <? if($action=="edit"){
  	...
  	if($extraField->datatype=="selection"){
  		...
  		$languages=request("SELECT $TBL_gen_languages.id,$TBL_gen_languages.image_file,$TBL_gen_languages.locales_folder,$TBL_catalogue_info_extrafields.selection_values FROM `$TBL_gen_languages`,`$TBL_catalogue_info_extrafields`  WHERE $TBL_gen_languages.used ='Y' and $TBL_catalogue_info_extrafields.id_language=$TBL_gen_languages.id and $TBL_catalogue_info_extrafields.id_extrafield=$idExtraField ORDER BY $TBL_gen_languages.id!='$used_language_id',$TBL_gen_languages.default",$link);
