...
if($idFaq){
	$cancelLink = "./mod_faq.php?action=edit&idFaq=$idFaq";
}
if($idCat){
...
if($nbCategories) {
	...
	if($idFaq){
		if($action=="addProduct"){
			echo showProducts($tableCategories,0,$idPrdCat,0,"./mod_faq.php?action=addProduct&idFaq=$idFaq&idProduct=","./list_products.php?action=addProduct&idFaq=$idFaq&idPrdCat=",$link);
			echo "<SCRIPT LANGUAGE='JAVASCRIPT'>location.href='#$idPrdCat'</SCRIPT>";
		}
		if($action=="addCat"){
			echo showPrdCategories($tableCategories,0,0,"./mod_faq.php?action=addPrdCat&idFaq=$idFaq&idPrdCat=",false);
		}
	}
	if($idCat){
		if($action=="addProduct"){
