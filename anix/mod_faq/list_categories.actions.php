...
if($action=="moveup"){
	...
	if(!$errors){
		$request=request("SELECT * from $TBL_faq_categories where id='$idCat'",$link);
	...
	if(!$errors){
		$request=request("SELECT * from $TBL_faq_categories where id_parent='".$category->id_parent."' and ordering='".($category->ordering-1)."'",$link);
		if(mysql_num_rows($request)){
			$upCategory=mysql_fetch_object($request);
		} else {
			$errors++;
			$errMessage.=_("La catégorie est déjà au plus haut niveau.")."<br />";
		}
	}
	if(!$errors){
		request("UPDATE $TBL_faq_categories set ordering='".($category->ordering-1)."',modified_on='".getDBDate()."',modified_by='$anix_username' where id='".$category->id."'",$link);
		if(mysql_errno()){
			$errors++;
	...
	if(!$errors){
		request("UPDATE $TBL_faq_categories set ordering='".$category->ordering."' where id='".$upCategory->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est produite lors de la mise à jour de l'ordre de la catégorie.")."<br />";
		}
...
if($action=="movedown"){
	...
	if(!$errors){
		...
		if(mysql_num_rows($request)){
			...
			$errors++;
			$errMessage.=_("La catégorie est déjà au plus bas niveau.")."<br />";
		}
	}
	if(!$errors){
		request("UPDATE $TBL_faq_categories set ordering='".($category->ordering+1)."',modified_on='".getDBDate()."',modified_by='$anix_username' where id='".$category->id."'",$link);
	...
	if(!$errors){
		request("UPDATE $TBL_faq_categories set ordering='".$category->ordering."' where id='".$downCategory->id."'",$link);
		if(mysql_errno()){
			$errors++;
			$errMessage.=_("Une erreur s'est produite lors de la mise à jour de l'ordre de la catégorie.")."<br />";;
		}
...
if($action=="deletecat"){
	...
	if(!$errors){
		$request=request("SELECT deletable,ordering,id_parent from $TBL_faq_categories where id=$idCat",$link);
		if(mysql_num_rows($request)){
	...
	if(!$errors && $_POST["method"]=="delete"){
		...
		if(!$moveto){
			$errors++;
			$errMessage=_("Vous n'avez pas spécifié dans quelle catégorie vous souhaitez déplacer les questions et sous-catégories.")."<br />";
		}
		if(!$errors){
			//get the target category max ordering
			$maxCatOrder = getMaxCategoryOrder($moveto,$link)-1;
			//update the subcategories ordering
			request("UPDATE $TBL_faq_categories set ordering=ordering+$maxCatOrder where id_parent='$idCat'",$link);
			// Move the subcategories to the target
			request("UPDATE $TBL_faq_categories set id_parent='$moveto' where id_parent='$idCat'",$link);
			if(mysql_errno($link)){
				$errors++;
		...
		if(!$errors){
			//get the target category max faq ordering
			$maxFaqOrder= getMaxFaqOrder($moveto,$link)-1;
			//update the ordering
			request("UPDATE $TBL_faq_faq set id_category=$moveto,ordering='ordering+$maxFaqOrder',modified_on='".getDBDate()."',modified_by='$anix_username' where id_category=$idCat",$link);
			if(mysql_errno($link)){
				$errors++;
				$errMessage.=_("Une erreur s'est produite lors du déplacement des questions.")."<br />";
			}
		}
		if(!$errors){
			//update orderings from where we are deleting
			request("UPDATE $TBL_faq_categories set ordering=ordering-1 where id_parent=$catParent and ordering > $catOrdering",$link);
			if(mysql_errno($link)){
				$errors++;
		...
		if(!$errors){
			//Delete the category
			request("DELETE from $TBL_faq_info_categories where id_faq_cat='$idCat'",$link);
			if(mysql_errno($link)){
				$errors++;
		...
		if(!$errors){
			//Delete the category
			request("DELETE from $TBL_faq_categories where id=$idCat",$link);
			if(mysql_errno($link)){
				$errors++;
